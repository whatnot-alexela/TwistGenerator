...
| 6 | Telegram Stars payments | **done** (stub-tested; no real Stars purchase has been made) |
| 7 | Everything deployment needs that does not need the server: Alembic migrations, Dockerfile and compose stack, backup script, `docs/DEPLOY.md`, generated `docs/methodology.html` | **done** |
| 8 | Deployment to the European VPS, methodology page on `kiloslov.ru`, stage-1 acceptance testing | blocked — the server has not been bought yet |
...
Milestone 7 exists because the server was not yet bought when the rest was
finished. It contains everything that can be prepared without one, so that
deployment is `git clone`, fill in `.env`, `docker compose up`. What it cannot
contain is the only thing that turns the cost estimates in §7 into
measurements: a real API call.

### 13.1 The methodology page

`docs/methodology.html` is generated by `scripts/build_page.py` from the same
`methodology/` files the prompt is assembled from, so the page cannot drift
from what the bot uses. The generation matrix on it is rendered from the
paradox rule rather than transcribed, and every catalogued formula gets a card
with its examples and references. Corrections apply; redactions do **not** —
those remove the author's asides from the *prompt*, and his book is his to
publish in full. CI rebuilds the page and fails if the committed file is
stale.
...
| 2 | Final pack prices, re-derived from measured spend | author | opening payments; defaults in §7.4 are usable as-is |
| 3 | Upload `docs/methodology.html` to `kiloslov.ru` | author | milestone 8 |
| 4 | European VPS provisioned, token and key installed | author | milestone 8 |
